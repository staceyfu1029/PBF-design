...
cs.list <- c(1,3)
method.list <- c("PoP","BOIN","Keyboard")
...
dose.labs <- c("K=4, N=30", "K=6, N=36", "K=6, N=60","K=6, N=96")
names(dose.labs) <- c("30", "36", "60", "96")
...
p2 <- ggplot(dt2, aes(x = -y, y = method, group=method, fill = method)) +
  ...
  facet_grid(phi+cohortsize~n, switch = "y",
             labeller = labeller(n = dose.labs, cohortsize = cs.labs, phi=phi.labs))
...
dat_text$method <- rep(c("BOIN","Keyboard"),each=24)

for(i in 1:dim(dat_text)[1]){
  temp <- -dt2$y[dt2$K==dat_text$K[i] & dt2$n==dat_text$n[i] & dt2$phi==dat_text$phi[i] & dt2$cohortsize==dat_text$cohortsize[i] & dt2$method==dat_text$method[i]]
  x <- mean(temp>0)
  dat_text$label[i] <- paste(round(100*x, 0), "%", sep="")
}

p3 <- p2+  annotate(
  geom = "curve", x = 0.12, y = 1.1, xend = 0.23, yend = 1.5, 
  curvature = .3, arrow = arrow(length = unit(2, "mm"))
) + annotate(
  geom = "curve", x = 0.12, y = 2.2, xend = 0.23, yend = 2.5, 
  curvature = .3, arrow = arrow(length = unit(2, "mm")),
  color = "#5D9693"
)+
  geom_text(
    data    = dat_text,
    mapping = aes(x = 0.24, y = method, label = label, color=method),
    vjust = -2.8
